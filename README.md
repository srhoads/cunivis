...
# How to make an `R` library: starting from scratch!
...
Our `NAMESPACE` file should look like this now...
```r
...
Our new `man` directory has a new `.Rd` file for our new `ggplot_lm` function! That `.Rd` file should have contents like this:
`man/ggplot_lm.Rd`
...
The installation output will look something like this...
...
`R` code used to create this example plot:
```r
...
# <em style="color:red">IGNORE/MISC...</em>

Example of the first couple lines of the `iris` dataframe we used in our example(s):
...
- SYNTAX FORMATTING SUPPORTED LANGUAGE FOR GITHUB MARKDOWNS:
